...
As an aside, I wanted to briefly mention one of the two Outstanding Main Track Award winners at NeurIPS 2023, “Are Emergent Abilities of Large Language Models a Mirage?” by Schaeffer, et al. https://arxiv.org/abs/2304.15004. In the area of AI safety and alignment, emergent abilities have been a concern, and especially as part of the discourse with the rise of capable LLMs this past year and their abilities to do well on human benchmark tests like the SAT. However, the authors of the paper demonstrate that “emergent abilities appear due to the researcher's choice of metric rather than due to fundamental changes in model behavior with scale. Specifically, nonlinear or discontinuous metrics produce apparent emergent abilities, whereas linear or continuous metrics produce smooth, continuous predictable changes in model performance.” It seems smooth metrics may also sometimes be desirable.
...
Here’s an example for all the code needed to fine-tune a vision model on a pets dataset within a couple minutes on a single GPU and achieve pretty good accuracy:
...
Example from https://arxiv.org/pdf/2002.04688.pdf
...
Create a validation dataset. Think about whether it makes sense to choose a random reshuffle of the training dataset or if certain factors matter like time or contextual clues that can be confounding factors.
...
Pick a batch size that fits the GPU. The GPU can run out of memory if using a larger model and large batch size. If training is CPU bounded, try resizing to smaller, approximate data. And if GPU memory allows, try fine-tuning a better, larger pre-trained model basically for free.
Train a model for a few epochs several times and see if the results are stable so we can know if improvements we’re seeing are due to the changes we’ve made.
Set up code such that we can quickly try different data processing and training parameters
Try out different ideas and see if they improve results. Some ideas are:
...
Data augmentation and other ideas listed above
Once the best ideas have been identified, scale up the data to the original size and train for more epochs.
Retrain on entire training set and use cross-validation to get more minor improvements, and consider ensembling if there aren’t strong constraints on inference time.

Check out https://www.kaggle.com/code/jhoward/iterate-like-a-grandmaster for more details.
