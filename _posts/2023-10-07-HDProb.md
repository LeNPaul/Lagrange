...
A set ${ T \subseteq \mathbb{R} ^n }$ is convex if for all ${ x, y \in T }$ the segment ${ [x,y] = \lbrace x + t(y-x) : t \in [0,1] \rbrace \subseteq T }.$ Intersection of any family of convex sets is convex. 

Obs: Let ${ T \subseteq \mathbb{R} ^n }.$ Any ${ z \in \text{conv}(T) }$ can be written as ${ z = \sum _{1} ^{m} \lambda _i z _i ,}$ where ${ \lambda \geq 0, \sum _{1} ^{m} \lambda _i = 1, z _i \in T }.$   
Pf: 
